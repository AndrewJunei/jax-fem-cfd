...
def _cg_solve(A, b, x0=None, *, maxiter, tol=1e-5, atol=0.0, M=_identity):
    ...
    x_final, _, _, _, iters = lax.while_loop(cond_fun, body_fun, initial_value)

    return x_final, iters
...
def _bicgstab_solve(A, b, x0=None, *, maxiter, tol=1e-5, atol=0.0, M=_identity):
  ...
  x_final, _, _, _, _, _, _, _, k_final = lax.while_loop(cond_fun, body_fun, initial_value)
...
def _gmres_batched(A, b, x0, unit_residual, residual_norm, ptol, restart, M):
    ...
    carry = (V, H, False, 0)
    V, H, _, k = lax.while_loop(loop_cond, arnoldi_process, carry)
    ...
    residual = M(_sub(b, A(x)))
    unit_residual, residual_norm = _safe_normalize(residual)
    return x, unit_residual, residual_norm, k
...
def _gmres_solve(A, b, x0, atol, ptol, restart, maxiter, M, gmres_func):
    ...
    def cond_fun(value):
        _, k, _, residual_norm, _ = value
        return jnp.logical_and(k < maxiter, residual_norm > atol)

    def body_fun(value):
        x, k, unit_residual, residual_norm, total_iters = value
        x, unit_residual, residual_norm, iters = gmres_func(
            A, b, x, unit_residual, residual_norm, ptol, restart, M)
        return x, k + 1, unit_residual, residual_norm, total_iters + iters

    initialization = (x0, 0, unit_residual, residual_norm, 0)
    x_final, _, _, _, total_iters = lax.while_loop(cond_fun, body_fun, initialization)

    return x_final, total_iters
...
def gmres(A, b, x0=None, *, tol=1e-5, atol=0.0, restart=20, maxiter=None,
    ...
    x, iters = lax.custom_linear_solve(A, b, solve=_solve, transpose_solve=_solve, has_aux=True)
