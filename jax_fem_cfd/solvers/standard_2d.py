""" 2D incompressible, Newtonian Navier Stokes with no body force
"""
import jax
import jax.numpy as jnp
from ..core.linear_shape_functions import quad_2d_shape_functions, linear_1d_shape_functions
from ..core.element_calc import *
from ..core.element_mult import stabilized_visc_conv_mult_2d, gradient_element_mult_2d, \
                                divergence_element_mult_2d, laplacian_element_mult
...
def setup_solver(node_coords, nconn, bconn, normal_vec, rho, not_gamma_p, gamma_d, U_d):
    ...
    wq, N, Nderiv = quad_2d_shape_functions(node_coords[nconn[0]])

    wq_1d, N_1d, _ = linear_1d_shape_functions(node_coords[bconn[0]])

    M = lumped_mass_matrix(nconn, rho, wq, N, nn)
    M = jnp.concatenate([M, M])
    Le = laplacian_element_calc(wq, Nderiv)
    Ge = gradient_element_calc(wq, N, Nderiv, ndim=2)

    F = u_dot_n_boundary_integral_1d(bconn, normal_vec, gamma_d, U_d, wq_1d, N_1d, nn)
...
def provisional_velocity(U_n, P_n, M, Le, Ge, Ce_all, Se_all, mu, dt, nconn, gamma_d, U_d, nn):
    ...
    """
    def lhs_operator(U):
        result = M * U / dt + stabilized_visc_conv_mult_2d(U, Ce_all, Se_all, Le, mu, nconn, nn)

        mask = jnp.zeros_like(U, dtype=bool).at[gamma_d].set(True)
        result = jnp.where(mask, U, result)
        return result
    
    rhs = M * U_n / dt - gradient_element_mult_2d(P_n, Ge, nconn, nn)
...
def update_pressure(U_star, P_n, Le, Ge, rho, dt, nconn, not_gamma_p, F, F_idx, nn):
    ...
    rhs = (rho/dt) * divergence_element_mult_2d(U_star, Ge, nconn, nn)
...
def four_step_timestep(U_n, U_n_1, P_n, M, Le, Ge, rho, mu, h, dt, nconn, wq, N, Nderiv, 
    ...
    Ce_all = convection_element_calc(nconn, rho, U_double_star, wq, N, Nderiv, nn, ndim=2) 
    ...
    U_star =  U_hat + dt * gradient_element_mult_2d(P_n, Ge, nconn, nn) / M
    ...
    U =  U_star - dt * gradient_element_mult_2d(P, Ge, nconn, nn) / M
